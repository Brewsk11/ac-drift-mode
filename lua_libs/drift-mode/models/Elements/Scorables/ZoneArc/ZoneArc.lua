...
function ZoneArc.__deserialize(data)
    local S = require('drift-mode.serializer')
    return ZoneArc(
...
---@private
function ZoneArc:recalculatePolygon()
...
function ZoneArc:setDirty()
    self:realignZoneArcPointOnTrack()
...
function ZoneArc:getOutsideLine()
    return self.outsideLine
end

function ZoneArc:getInsideLine()
    return self.insideLine
end

function ZoneArc:setOutsideLine(outside_line)
    self.outsideLine = outside_line
    self:setDirty()
end

function ZoneArc:setInsideLine(inside_line)
...
---@return boolean
function ZoneArc:isInZoneArc(point, custom_origin)
...
function ZoneArc:isSegmentInZoneArc(segment, custom_origin)
    local is_head_in_zone = self:isInZoneArc(segment.head, custom_origin)
    local is_tail_in_zone = self:isInZoneArc(segment.tail, custom_origin)
...
---@param point Point
---@return table
function ZoneArc:shortestCrossline(point)
...
function ZoneArc:recalculateBoundingBox()
    -- TODO: Add padding for inZoneArc detection
...
local function test()
    ...
    local zone = ZoneArc("test", outside, inside, 0)
    local custom_origin = Point(vec3(23.45, 0, 51.23))

    local segment = Segment(
        Point(vec3(2, 0, 0)),
        Point(vec3(2, 0, 2)))
    Assert.Equal(zone:isSegmentInZoneArc(segment, custom_origin), 0.0, "ZoneArc.isZoneArcInSegment() test failed")

    segment = Segment(
        Point(vec3(0.1, 0, 0.1)),
        Point(vec3(0.5, 0, 0.8)))
    Assert.Equal(zone:isSegmentInZoneArc(segment, custom_origin), 1.0, "ZoneArc.isZoneArcInSegment() test failed")

    segment = Segment(
        Point(vec3(0.5, 0, 0.5)),
        Point(vec3(0.5, 0, 2.5)))
    Assert.Equal(zone:isSegmentInZoneArc(segment, custom_origin), 0.25, "ZoneArc.isZoneArcInSegment() test failed")

    segment = Segment(
        Point(vec3(0.5, 0, 0.5)),
        Point(vec3(0.5, 0, 4.5)))
    Assert.Equal(zone:isSegmentInZoneArc(segment, custom_origin), 0.125, "ZoneArc.isZoneArcInSegment() test failed")
